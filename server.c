...
int main(int argc, char **argv)
{
	...
	while(1)
	{
		...
		int bad = 0;
		int invalid = 0;
		//use bad as a state toggle, if bad is 1 and sees another opening parenthesis, invalid is set
		//if invalid is ever 1, the expression will always be returned invalid
		for(l = 0; l < strlen(buf); l++)
		{
			if(buf[l] == '(')
			{
		...
		for(l = 0; l < strlen(buf); l++)
		{
			...
			//check only after with a subtraction, just in case of leading negative numbers
			if(buf[l] == '-')
			{
				if(buf[l+1] == ')')
				{
		...
		else
		{
			...
			for(l = 0; l < strlen(buf); l++)
			{
				...
				if(in == 1)
				{
					//scan for these cases, and puts the held characters into the integer array
					...
					//special cases for -, as it could be a negative sign or subtraction sign
					else if(buf[l] == '-')
					{
						//if its a pow function, add the negative sign to the char holder, as it makes the integer negative
						if(buf[l-1] == '^')
						{
                                                	intHolder[hCount] = buf[l];
                                                	hCount++;
						}
						//if its not pow, treat the - as a subtraction symbol
						else
						{
							sscanf(intHolder, "%d", &opInt[iCount]);
                                                	hCount = 0;
					...
					//specific case if it is an exponential function
					else if(buf[l] == 'e')
					{
						in = 1;
					}
					//if it is a number, add that number to the char holder
					else
					{
						intHolder[hCount] = buf[l];
						hCount++;
						//if the bracket ends there, convert the char holder, and put it in the integer array
						if(buf[l+1] == ')')
						{
							in = 0;
							sscanf(intHolder, "%d", &opInt[iCount]);
				...
				//checks to see if the first number in the parenthesis is a negative
				if(buf[l] == '(')
                                {
                                        in = 1;
                                        if(buf[l+1] == '-')
			...
			op = opChar[3];
			//check for last possible, single integer operation
			if(op != 'p')
			{
				if(op == '+')
                                {
